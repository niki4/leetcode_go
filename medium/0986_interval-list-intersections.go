...
func intervalIntersection(firstList [][]int, secondList [][]int) [][]int {
	...
	for i < len(firstList) && j < len(secondList) {
		...
		if aStart <= bEnd && bStart <= aEnd { // criss-cross
			result = append(result, []int{max(aStart, bStart), min(aEnd, bEnd)}) // intersection
		}

		if aEnd <= bEnd { // can move A because B lasts longer
			i++
		} else { // vice versa
			j++
		}
